...
string Sala::tipoSala()
{
	return nomeSala;
}
void Sala::DefineQtdPoltrona()  // define nPoltrona para cada tipo de sala
{
...
bool Sala::EscolhePoltrona(string Nome_Poltrona, int *opi, int i)
{
	for( unsigned int j = 0 ; j < Polt[i].size(); j++)
	{
		for( unsigned int k = 0 ; k < Polt[i][j].size(); k++)
...
string Sala::Nomepoltrona(int i, int j, int k)
{
